...
export const updateUserInfo = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { name, email } = req.body as TUpdateUserInfo;
      const userId = req.user?._id;
...
export const updateProfile = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!avatar) {
        return next(new ErrorHandler("Please provide your image", 400));
      }
      ...
      if (avatar && user) {
        //if user already have an avatar
...
export const updateUserRole = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!id || !role) {
        return next(new ErrorHandler("Invalid user id or user role", 400));
      }
